...
public class UserImpl extends AbstractVerifyingElement implements Serializable,
	...
	public String getShortRole() {
		if (RoleController.ADMIN.equalsIgnoreCase(this.role)) {
			return "role.adm";
		}

		if (RoleController.PUBLISHER.equalsIgnoreCase(this.role)) {
			return "role.pub";
		}

		if (RoleController.ADVERTISER.equalsIgnoreCase(this.role)) {
			return "role.adv";
		}

		if (RoleController.ADVERTISERPUBLISHER.equalsIgnoreCase(this.role)) {
			return "role.advpub";
		}

		if (RoleController.HOSTEDSERVICE.equalsIgnoreCase(this.role)) {
			return "role.hosted";
		}

		if (RoleController.GUEST.equalsIgnoreCase(this.role)) {
			return "role.guest";
		}
	...
	public Integer getAccountMoney() {
		Account account = MoneyManager.loadUserAccount(this.getId());
