...
public class BannerManagementService {
	...
	public void getBannersFromActiveCampaigns(AdserverModel adserverModel) {
		List<IMappable> activeCampaigns = (List<IMappable>) hibernateEntityDao
				.viewWithCriteria(CampainImpl.class, "stateId",
						ConfigurationConstants.DEFAULT_ACTIVE_CAMPAIN_STATE_ID,
						"campainId");
		List<BannerImpl> activeBanners = new ArrayList<BannerImpl>();
		List<BannerImpl> mactiveBanners = new ArrayList<BannerImpl>();
		...
		for (IMappable campaign : activeCampaigns) {
			...
			if (!camp.getUserDefineCampainStateId().equals(
					ConfigurationConstants.DEFAULT_USER_DEFAIN_CAMPAIN_STATE)) {
				continue;
			}
