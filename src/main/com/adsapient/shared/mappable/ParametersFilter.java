...
import com.adsapient.shared.mappable.DynamicParameter;
...
public class ParametersFilter implements FilterInterface, IMappable {
	...
	public Collection getAllParameters() {
        ...
        if (allParameters == null) {
			allParameters = hibernateEntityDao.viewAll(ParameterImpl.class);
		}
	...
	public void addNew() {
        ...
        HibernateEntityDao hibernateEntityDao = (HibernateEntityDao) ContextAwareGuiBean.getContext().getBean("hibernateEntityDao");
        parametersFilterId = (Integer) hibernateEntityDao.save(this);
			...
			if (param.getType() == param.PREDEFINED_TYPE_ID) {
				buffer
						.append("<td class=\"tabledata\">"
								+ I18nService.fetch("possible.choices", request)
								+ "</br><select name=\""
								+ param.getName()
				...
				buffer.append("title='"
						+ I18nService.fetch("move.all.options.to.target", request)
						+ "' ");
				buffer
				...
				buffer.append("title='"
						+ I18nService.fetch("move.selected.options.to.target.list",
								request) + "' ");
				buffer
				...
				buffer.append("<input type=\"button\" value=\"&lt;\"");
				buffer.append("title='"
						+ I18nService.fetch("delete.selected.options.from.target",
								request) + "' ");
				buffer
				...
				buffer.append("title='"
						+ I18nService.fetch("delete.all.options.from.target", request)
						+ "' ");
				...
				buffer
						.append("<td class=\"tabledata\">"
								+ I18nService.fetch("actual.selection", request)
								+ "</br><select name=\""
								+ param.getName()
								+ "\" multiple=\"multiple\" class=\"uiOptionParameters_target\" id=\""
		...
		if ((parameters != null) && (parameters.size() > 0)) {
			for (Iterator parametersIterator = getAllParameters().iterator(); parametersIterator
				...
				if (dparam == null) {
                    ...
                    HibernateEntityDao hibernateEntityDao = (HibernateEntityDao) ContextAwareGuiBean.getContext().getBean("hibernateEntityDao");
                    hibernateEntityDao.save(dparam);
	...
	public void reset() {
        HibernateEntityDao hibernateEntityDao = (HibernateEntityDao) ContextAwareGuiBean.getContext().getBean("hibernateEntityDao");
        hibernateEntityDao.removeWithCriteria(ParametersFilter.class,
				"parameterFilterId", this.getParametersFilterId());

		ParametersFilter filter = (ParametersFilter) hibernateEntityDao
	...
	public void update(HttpServletRequest request, ActionForm actionForm,
		...
		for (Iterator parametersIterator = getAllParameters().iterator(); parametersIterator
			...
			if (value != null) {
				Collection dparams = hibernateEntityDao.viewWithCriteria(
				...
				if (enableHibernateupdate) {
					hibernateEntityDao.saveOrUpdate(dparam);
				}
			}
		}
