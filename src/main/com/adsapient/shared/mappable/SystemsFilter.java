...
import com.adsapient.shared.mappable.ReferrersElement;
...
public class SystemsFilter implements FilterInterface, IMappable {
	...
	public void addNew() {
		...
		for (Iterator iter = FiltersService.BrowsersMap.entrySet().iterator(); iter
				.hasNext();) {
		...
		for (Iterator iter = FiltersService.SystemsMap.entrySet().iterator(); iter
				.hasNext();) {
			Map.Entry entry = (Map.Entry) iter.next();
		...
		for (Iterator iter = FiltersService.LangsMap.entrySet().iterator(); iter
				.hasNext();) {
			Map.Entry entry = (Map.Entry) iter.next();
		...
		systemsFilterId = (Integer) hibernateEntityDao.save(this);
	...
	public void reset() {
        HibernateEntityDao hibernateEntityDao = (HibernateEntityDao) ContextAwareGuiBean.getContext().getBean("hibernateEntityDao");
        hibernateEntityDao.removeWithCriteria(ReferrersElement.class,
				"systemsFilterId", this.getSystemsFilterId());

		SystemsFilter filter = (SystemsFilter) hibernateEntityDao.loadObject(
	...
	public void update(HttpServletRequest request, ActionForm actionForm,
		...
		if ("remove".equalsIgnoreCase(form.getReferrerEngineAction())) {
			...
			if ((form.getReferrerId() != null)
				...
				hibernateEntityDao.removeObject(ReferrersElement.class,
