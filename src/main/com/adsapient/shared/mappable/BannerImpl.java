...
public class BannerImpl extends CampainImpl implements Banner, Cloneable {
	...
	public BannerImpl(BannerUploadActionForm form) {
		super();
	...
	public CampainImpl getCampaign() {
		...
		logger.info("get campain by Hibernate load.Campain id is "
				+ this.getCampainId());
		campain = (CampainImpl) hibernateEntityDao.loadObject(CampainImpl.class,
				this.getCampainId());
	...
	public String getStatusText(HttpServletRequest request) {
		...
		if (this.status == STATUS_ENABLED) {
			result = I18nService.fetch("enabled.label", request);
		}

		if (this.status == STATUS_DISABLED) {
			result = I18nService.fetch("disabled", request);
		}

		if ((this.status == STATUS_DEFAULT)
				|| (this.status == STATUS_PUBLISHER_DEFAULT)) {
			result = I18nService.fetch("banner.default", request);
		}
	...
	public boolean isSuitable(PlacesInterface places,
		...
		if (this.getCampaign().getStateId() == AdsapientConstants.DEFAULT_CAMPAIN_STATE_ID) {
			return true;
		}

		if ((places.getUserId().equals(this.getUserId()))
				&& (this.getCampaign().getStateId() != AdsapientConstants.DEFAULT_CAMPAIN_STATE_ID)) {
	...
	public boolean add(BannerUploadActionForm form) {
        HibernateEntityDao hibernateEntityDao = (HibernateEntityDao) ContextAwareGuiBean.getContext().getBean("hibernateEntityDao");
	...
	public boolean resultView(ActionForm actionForm, HttpServletRequest request) {
		...
		Size size = (Size)hibernateEntityDao.loadObject(Size.class, this
				.getSizeId());
	...
	public void update(BannerUploadActionForm form,
        ...
        HibernateEntityDao hibernateEntityDao = (HibernateEntityDao) ContextAwareGuiBean.getContext().getBean("hibernateEntityDao");
        Size size = (Size)hibernateEntityDao.loadObject(Size.class, form
				.getSizeId());
		...
		if (!"".equalsIgnoreCase(form.getTheFile().getFileName())
			...
			if (form.getTheFile().getFileSize() > size.getMaxBannerSize()) {
				...
				UploadService.removeFile(form.getFile());
