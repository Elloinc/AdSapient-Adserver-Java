...
import com.adsapient.util.link.PublisherLinkHelper;
...
public class PlacesEditActionForm extends ActionForm
    ...
    public String getBannerSrc() {
        if ((this.placesId != null) && (this.siteId != null)) {
            return PublisherLinkHelper.generatePlaceAdCode(placesId,
                this.request);
        } else {
    ...
    public String getDemoHref() {
        ...
        PlacesImpl places = (PlacesImpl) MyHibernateUtil.loadObject(PlacesImpl.class,
                this.getPlacesId());

        Size placeSize = (Size) MyHibernateUtil.loadObject(Size.class,
                places.getSizeId());
    ...
    public Collection getLoadingTypeCollection() {
        this.loadingTypeCollection = LabelValueUtil.fillTargetingCollection(AdsapientConstants.LOADING_TYPE_TYPE,
                false, this.request);
    ...
    public Collection getPlaceCollection() {
        if (placeCollection == null) {
            placeCollection = LabelValueUtil.fillSitePlacesValueLabel();
        }
    ...
    public Collection getPlaceTypeCollection() {
        this.placeTypeCollection = LabelValueUtil.fillTargetingCollection(AdsapientConstants.PLACE_TYPE_TYPE,
                false, this.request);
    ...
    public Collection getPossibleCounts() {
        this.possibleCounts = LabelValueUtil.fillCustomWeightCollection(1, 10);
    ...
    public Collection getSitesCollection(HttpServletRequest request) {
        if (SitesCollection.isEmpty()) {
            SitesCollection = LabelValueUtil.getSitesCollection(request);
        }
    ...
    public Collection getTargetWindowCollection() {
        this.targetWindowCollection = LabelValueUtil.fillTargetingCollection(AdsapientConstants.TARGET_WINDOW_TYPE,
                true, request);
        ...
        if (catsString != null) {
            ...
            for (String catId : catIds) {
                try {
                    ...
                    Category category = (Category) MyHibernateUtil.loadObject(Category.class,
                            catIdInt);
