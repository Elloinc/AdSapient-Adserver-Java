		...
		if (filterEntity.getBannerId() != null) {
			for (BannerImpl banner : getBannersPool())
				if (banner.getId().equals(filterEntity.getBannerId())) {
					if (banner.getFilters() != null) {
						for (ListIterator<FilterInterface> iter = banner
								.getFilters().listIterator(); iter.hasNext();) {
							FilterInterface filter = iter.next();

							if (((IMappable) filter).getId().equals(
									entity.getId())
									&& filter.getClass().getName().equals(
											className)) {
								iter.set(filterEntity);
			...
			CampainImpl campaign = (CampainImpl) campaignsMap.get(filterEntity
					.getCampainId());
