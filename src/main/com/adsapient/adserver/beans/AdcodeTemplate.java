...
public class AdcodeTemplate {
    Logger logger = Logger.getLogger(AdcodeTemplate.class);
	...
	public String getAdCodeFromParameters(Map<String, Object> params) {
		...
		for (String paramLabel : params.keySet()) {
            String paramName = new StringBuffer("\\{").append(paramLabel).append("\\}").toString();
            ...
            try{
            	ret = ret.replaceAll(paramName, paramValue);
            }catch (Exception e) {
				logger.error(paramLabel + "\t" + paramValue, e);
			}
		}
		return ret;
	}
