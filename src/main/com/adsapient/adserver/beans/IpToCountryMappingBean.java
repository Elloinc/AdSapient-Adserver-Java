...
public class IpToCountryMappingBean {
    private static Logger logger = Logger.getLogger(IpToCountryMappingBean.class);

    private static final String ABBR_ADDRES_RESERVED_BASE = "ZZ";

	private static final String databaseName = "countryIp";

    private Map<String, String> cachedSearchResults = new HashMap<String, String>();
	...
	private void importCSV() {
		...
		try {
			...
			logger.error("Cannot import IpToCountry.csv:"+e.getMessage());
    ...
    public String search(String key) {
        ...
        if (ips == null) {
            logger.error("IP database is not initialized");
            return AdsapientConstants.COUNTRY_ABBR_ADDRES_ANONIMOUS;
        }
        ...
        try {
            StringTokenizer tokens = new StringTokenizer(key, ".", false);
            long ip = 0L;
            int counter = 3;
        ...
        if (cachedSearchResults.containsKey(searchKey)) return cachedSearchResults.get(searchKey);
        try {
            DatabaseEntry theKey = new DatabaseEntry(searchKey.getBytes());
            DatabaseEntry theData = new DatabaseEntry();
            OperationStatus retVal = ipDatabase.get(null, theKey, theData, LockMode.DEFAULT);
            if (retVal.equals(OperationStatus.SUCCESS)) {
                IGeoIpEntry found = entryToObject(theData);
                cachedSearchResults.put(searchKey, found.getCountryAbbr2());
