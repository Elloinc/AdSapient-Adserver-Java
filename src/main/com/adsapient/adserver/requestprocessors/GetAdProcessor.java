...
public class GetAdProcessor extends AbstractAdsapientProcessor {
	static Logger logger = Logger.getLogger(GetAdProcessor.class);
	...
	public void doGet(Map<String, Object> requestParams,
		...
		try {
			...
			if (is == null) {
				logger
						.error("Banner file not found for:"
								+ ((banner == null) ? "null banner" : banner
										.getFile()));
				is = new FileInputStream(AdsapientConstants.PATH_TO_BANNERS
						+ adserverModel.getDefaultBanner().getFile());
			}

			bbs = new byte[is.available()];
			is.read(bbs);
			is.close();
