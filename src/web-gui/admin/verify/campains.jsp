<%@ page import="com.adsapient.api_impl.publisher.*,
							  ...
							  com.adsapient.api_impl.advertizer.*,
							  com.adsapient.util.financial.MoneyTransformer,
							  com.adsapient.api_impl.usermanagment.*,
                              com.adsapient.api_impl.statistic.advertizer.*" %>
...
<%UserImpl iterUser= (UserImpl) user;%>
	...
	<td class="tabledata-c"><nobr><%=iterUser.getName()%></nobr></td>
   	<td class="tabledata-c"><nobr><%=Msg.fetch(iterUser.getShortRole(),request)%></nobr></td>
    <%=FinancialLinksController.generateFinancialLink(iterUser,response)%>
    <td class="tabledata-c"><nobr><a class="tabledata" href="<%=response.encodeURL("viewAccount.do?action=accountView&userId="+iterUser.getId())%>"><%=MoneyTransformer.transformMoney(iterUser.getAccountMoney())%></a></nobr></td>
	...
	<td class="tabledata-c"><nobr><a class="tabledata" href="<%=response.encodeURL("profileEdit.do?action=remove&id="+iterUser.getId())%>"><img src="images/icons/remove.png" border="0" title=<bean:message key="remove"/>></a></nobr></td>
		</tr>
	</logic:iterate>
		</table></td>
	<td width="6" heigh="10" style="background-image: url(images/table5.gif);"></td>
...
<tr><td>
	...
	<td height="10" class="tabledata-c"><nobr><%=FinancialLinksController.generateRates(session,site,response)%></nobr></td>	
	<td height="10" class="tabledata-c"><nobr><%=entity.getCtr() %></nobr></td>
