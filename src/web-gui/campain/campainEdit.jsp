...
<tr><td>
	...
	<td width="100%" class="tableheader"><%if ("add".equals(form.getAction())) {
	 out.println(i18nService.fetch(form.getAction()+".campaign.title", request));
	  } else {
	 out.println(i18nService.fetch("edit.campaign.title",request)+form.getCampainId()); }%></td>
...
--%>
 ...
 <%--   
	...
	<td width="5" heigh="10" style="background-image: url(images/table4.gif);"></td>
	<td ></td>
	<td height="10" ><table border="0" width="100%" cellspacing="5"><tr><td ><input type="submit" value='<%=i18nService.fetch(form.getAction()+".campaign.button",request)%>'>
	<%if ("edit".equals(form.getAction())){%>
	&nbsp;&nbsp;&nbsp;<input type="button" value='<bean:message key="remove"/>'
	onclick="if( window.confirm('<%=i18nService.fetch("delete.confirm",request)%>')) {document.location='<%=response.encodeURL("campainEdit.do?action=remove&campainId="+form.getCampainId())%>'}"> 
	<%}%>
	</td></tr></table></td>
	<td width="6" heigh="10" style="background-image: url(images/table5.gif);"></td>
	</tr>
