<%@ page import="com.adsapient.shared.mappable.UserImpl,
                 ...
                 com.adsapient.gui.forms.BannerUploadActionForm" %>
        ...
        <td width="100%" class="tableheader"><%if (form.getBannerId() != null) {
            if(form.getBannerId() != 0){
            	if (form.getStatus() == BannerImpl.STATUS_DEFAULT || form.getStatus() == BannerImpl.STATUS_PUBLISHER_DEFAULT) 
            		out.println(i18nService.fetch("banner.edit.default.title", request) + form.getBannerId());
            	else
                	out.println(i18nService.fetch("banner.edit.title", request) + form.getBannerId());
            }
            else
                out.println(i18nService.fetch("banner.add.title", request));
        } else {
            out.println(i18nService.fetch("banner.add.title", request));
        }%></td>
	        ...
	        <html:select property="status"  styleClass="defType">
	            ...
	            <% } else {%>
	            	<option value=<%if (form.getStatus() == BannerImpl.STATUS_DEFAULT) out.println(BannerImpl.STATUS_DEFAULT);
						...
						<%out.println(i18nService.fetch("default", request)); %>
	            	</option>
	            <% } %>
            </html:select>
	        </td></tr></table>
    ...
    <td height="10" colspan="2" class="tabledata">
        <table border="0" width="100%" cellspacing="5"><tr><td class="maintext"><html:submit/>
            ...
            &nbsp;&nbsp;&nbsp;<input type="button" value='<bean:message key="remove"/>'
                                     onclick="if( window.confirm('<%=i18nService.fetch("delete.confirm",request)%>')) {document.location='<%=response.encodeURL("upload.do?action=removeBanner&bannerId="+form.getBannerId())%>'}">
            <%}%>
        </td></tr></table>
    </td>
</tr>
