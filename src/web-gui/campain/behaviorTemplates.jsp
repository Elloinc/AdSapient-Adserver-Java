...
<%@ page import="com.adsapient.api_impl.filter.BehaviorPattern"%>
...
<tr><td>
	...
	<td width="100%" class="tableheader"><bean:message key="behavior.patterns.list"/></td>
...
<logic:iterate  id="templateMy"  name="behaviorActionForm" property="templatesCollection">
<%BehaviorPattern pattern= (BehaviorPattern) templateMy;%>

		<tr>
	<td height="10" class="tabledata"><nobr><%=pattern.getId()%></nobr></td>
	<td height="10" class="tabledata"><nobr><%=pattern.getName()%></nobr></td>
	<td height="10" class="tabledata-c"><nobr><a class="tabledata" href='<%=response.encodeURL("behaviorAction.do?action=preview&id="+pattern.getId())%>'>
	 ...
	 <td height="10" class="tabledata-c"><nobr><a class="tabledata"
	  href='<%=response.encodeURL("behaviorAction.do?action=del&id="+pattern.getId())%>' onClick="return window.confirm('<%=(Msg.fetch("delete.confirm",session)+" "+pattern.getId())+%>');"><img src="images/icons/remove.png" border="0" title=<bean:message key="remove"/>></a></nobr></td>
	   </tr>
