...
<%StatisticEntity entity=banner.getStatistic();%>
	...
	<td height="10" class="tabledata-c"><nobr><a class="tabledata" href="<%=response.encodeURL(banner.getSettingsHref())%>"><img src="images/icons/edit.png" border="0" title=<bean:message key="edit"/>></a></nobr></td>
	 <td height="10" class="tabledata-c"><nobr><a class="tabledata" href="<%=response.encodeURL("totals.do?bannerId="+banner.getId())%>" onClick="return window.confirm('<%=(Msg.fetch("reset.confirm",session))%>');"><img src="images/icons/resetstats.gif" border="0" title='<bean:message key="reset"/>'/></a></nobr></td>
     <td height="10" class="tabledata-c"><nobr><%if (banner.getDeleteHref()!=null){%><a class="tabledata" href="<%=response.encodeURL(banner.getDeleteHref())%>" onClick="return window.confirm('<%=(Msg.fetch("delete.confirm",session)+" "+banner.getName())%>');"><img src="images/icons/remove.png" border="0" title=<bean:message key="remove"/>></a><%}%></nobr></td>
