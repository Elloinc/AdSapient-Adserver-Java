...
<tr><td>
    <table width="100%" cellspacing="0" cellpadding="0"><tr>
        ...
        <td width="100%"
            class="tableheader"><%=i18nService.fetch(form.getAction() + ".site.title", session)%> <%=(form.getSiteId() != null ? "#" + form.getSiteId().toString() : "")%></td>
...
<tr>
    ...
    <td height="10" >
        <html:form action="publisherEdit">
            ...
            <table border="0" width="100%" cellspacing="5"><tr><td >
                <input type="submit" value='<%=i18nService.fetch(form.getAction()+".site.button",request)%>'
                ...
                &nbsp;&nbsp;&nbsp;<input type="button" value='<bean:message key="remove"/>'
                                         onclick="selectionChanged();if( window.confirm('<%=i18nService.fetch("delete.confirm",request)%>')) {document.location='<%=response.encodeURL("publisherEdit.do?action=remove&siteId="+form.getSiteId())%>'}">
                <%}%>
            </td></tr></table>
        </html:form>
    </td>
