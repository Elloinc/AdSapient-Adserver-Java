...
<tr><td>
	...
	<tr>
		...
		<td  class="tabledata-c"><nobr><%=lhs.generateRates(session,site,response)%></nobr></td>
		<td  class="tabledata-c"><nobr><%=lhs.generateCPLRates(session,site,response)%></nobr></td>
        ...
        <td class="tabledata-c"><nobr><a class="tabledata" href="<%=response.encodeURL("totals.do?siteId="+site.getId())%>" onClick="return window.confirm('<%=(i18nService.fetch("reset.confirm",session))%>');"><img src="images/icons/resetstats.gif" border="0" title='<bean:message key="reset"/>'/></a></nobr></td>
		...
		<td  class="tabledata-c"><nobr><a class="tabledata" href="<%=response.encodeURL("publisherEdit.do?action=remove&siteId="+site.getSiteId())%>" onClick="return window.confirm('<%=(i18nService.fetch("delete.confirm",session)+" "+site.getSiteId())%>');"><img src="images/icons/remove.png" border="0" title=<bean:message key="remove"/>></a> </nobr></td>
		...
		<tr>
	<td height="10"  colspan="13"  class="tabledata"><table border="0" width="100%" cellspacing="5"><tr><td class="tabledata"><input type="submit" value='<%=i18nService.fetch("site.addsite.button",session)%>'></td></tr></table></td>
		</tr>
		</table></td>
